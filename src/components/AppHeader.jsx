  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-500 ${
      isTransparent 
        ...
        : 'bg-[#080B11]/90 backdrop-blur-xl border-b border-[#D4AF37]/15 text-[#f8fafc] shadow-[0_4px_30px_rgba(0,0,0,0.5)]'
    }`}>
      <div className="mx-auto flex max-w-[1380px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-6 lg:flex">
          {!isDetailPage && (
            <div className="relative">
              ...
              {localityOpen && (
                <div className="absolute left-0 top-[34px] z-40 w-[520px] overflow-hidden rounded-[16px] border border-slate-800 bg-[#080B11]/98 p-5 shadow-[0_25px_60px_-20px_rgba(0,0,0,0.8)] backdrop-blur-lg">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {bangaloreLocalities.map((loc) => (
                      <Link
                        ...
                        className="rounded-[8px] px-3 py-2 text-[13px] text-slate-300 transition hover:bg-[#D4AF37]/10 hover:text-[#D4AF37]"
                      >
                        {loc}
                      </Link>
                    ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button className={`relative rounded-full p-2 transition-colors duration-500 ${isTransparent ? 'text-white/80 hover:text-white' : 'text-slate-300 hover:text-[#D4AF37]'}`}>
          ...
          <button className={`rounded-full p-2 transition-colors duration-500 ${isTransparent ? 'text-white/80 hover:text-white' : 'text-slate-300 hover:text-[#D4AF37]'}`}>
            <UserCircle2 className="h-5 w-5" />
          </button>
          <button onClick={() => setMobileOpen(true)} className={`rounded-full p-2 transition-colors duration-500 lg:hidden ${isTransparent ? 'text-white/80 hover:text-white' : 'text-slate-300'}`}>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-[#080B11] text-slate-100 lg:hidden">
          <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
            <div className="flex h-10 items-center px-1 text-[18px] font-black text-[#D4AF37]">EastProject.in</div>
            <button onClick={() => setMobileOpen(false)}><X className="h-6 w-6 text-slate-300" /></button>
          </div>
          ...
          <div className="px-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {topLocalities.map((loc) => (
                <Link key={loc} to="/residential-properties-in-bangalore" onClick={() => setMobileOpen(false)} className="text-[14px] text-slate-300 py-1">{loc}</Link>
              ))}
            </div>
          </div>
        </div>
